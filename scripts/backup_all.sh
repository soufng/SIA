#!/usr/bin/env bash
# Backup unifié SIA : Mongo + Qdrant en un seul appel.
#
...
#   MONGO_DB          : nom de la base, défaut sia
...
#   BACKUP_ROOT       : répertoire racine, défaut /var/backups/sia
...
BACKUP_ROOT="${BACKUP_ROOT:-/var/backups/sia}"
BACKUP_KEEP="${BACKUP_KEEP:-14}"

MONGO_URI="${MONGO_URI:-mongodb://127.0.0.1:27017}"
MONGO_DB="${MONGO_DB:-sia}"
...
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] === SIA backup starting ==="
...
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] === SIA backup finished ==="
